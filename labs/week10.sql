...
END sylvester;
/
Develop an anonymous block that tests the warner_brother procedure by calling it with two sets of data, as qualifed below.
...
CREATE OR REPLACE
  ...
  BEGIN
    ...
    lv_user_id := get_system_user_id(pv_user_name);
    insert_contact(pv_first_name, pv_middle_name, pv_last_name, pv_contact_type, pv_account_number,
                   pv_member_type, pv_credit_card_number, pv_credit_card_type, pv_street_address,
                   pv_city, pv_state_province, pv_postal_code, pv_address_type, pv_country_code,
                   pv_area_code, pv_telephone_number, pv_telephone_type, lv_user_id);
